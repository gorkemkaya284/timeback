-- Call assess_risk after crediting offerwall event (record only; no auto-block).
...
BEGIN
  ...
  -- Risk assessment (record only; no auto-block for offerwall)
  PERFORM assess_risk('offerwall_event', p_event_id);
