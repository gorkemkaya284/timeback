-- ============================================
-- redeem_reward: integrate can_redeem, assess_risk, risk block reversal.
-- Keeps existing idempotency and FOR UPDATE; adds cooldown and stock_remaining.
-- ============================================
...
BEGIN
  ...
  -- Stock: use stock_remaining if present, else stock
  ...
  -- Risk assessment: if block, revert (reject + credit)
  v_risk := assess_risk('reward_redemption', v_redemption_id);
  IF v_risk.recommended_action = 'block' THEN
    UPDATE tb_reward_redemptions SET status = 'rejected' WHERE id = v_redemption_id;
    INSERT INTO points_ledger (user_id, delta, type, reason, ref_type, ref_id)
    VALUES (v_uid, v_variant.cost_points, 'credit', 'reward_redeem_reversal', 'redeem_reversal', v_redemption_id::text);
    RETURN jsonb_build_object('success', false, 'error', 'RISK_BLOCKED', 'message', 'Request blocked by risk rules');
  END IF;
